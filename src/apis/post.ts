...
export const useUpdatePost = (id: number) => {
  return useUpdate<Post, PostUpdate>(toUrl(ApiRoutes.Post, { id }), undefined, undefined, (old, data) => ({ ...old, ...data }));
};

// [DELETE] /api/posts/{id}
export const useDeletePost = () => {
  return useDelete<Post[], number>(toUrl(ApiRoutes.Post), undefined, undefined, (old, id) => old.filter((item) => item.id !== id));
};
