...
const updateTMobile = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    return res.status(200).send({ data: [], message: `Successfully update ${usimName}` });
  } catch {
    return res.status(500).send({ data: null, message: `Failed to update ${usimName}` });
  }
};
