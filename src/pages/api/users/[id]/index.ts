...
const getUser = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    return res.status(500).json({ data: null, message: `Failed to get user ${id}` });
  }
};
...
const updateUser = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    return res.status(500).json({ data: null, message: `User ${id} update failed` });
  }
};
  ...
  parseIP(req)
    .then(async (ip) => {
      ...
      try {
        ...
        return res.status(500).json({ data: null, message: `User ${id} deletion failed` });
      }
    })
    .catch(() => {
