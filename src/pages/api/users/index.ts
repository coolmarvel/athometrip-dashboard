...
const getUsersByPage = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    const users = await readUsers(sort as RequiredKeysOf<User>, order as Order, search as string);
    const slicedUsers = users.slice(Number(offset), Number(offset) + Number(limit));
...
const createUser = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    if (users.some((user) => user.email === email)) {
      return res.status(409).json({ data: null, message: 'Email already exists' });
    }
    ...
    return res.status(200).json({ data: newUser.id, message: `User ${newUser.id} created` });
  } catch {
    return res.status(500).json({ data: null, message: `User ${name} creation failed` });
  }
};
