...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  switch (req.method) {
    case 'POST':
      return updateNiagaraOneDayAthometrip(req, res);
    default:
...
const updateNiagaraOneDayAthometrip = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    return res.status(200).send({ data: [], message: `Successfully update ${tourName}` });
  } catch {
    return res.status(500).send({ data: null, message: `Failed to update ${tourName}` });
  }
};
