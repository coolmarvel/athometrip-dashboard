...
const getWollmanByPage = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    await ticketStore.fetchTicket(ticketName, `${url}/1/120?start_date=${startDate}&end_date=${endDate}`);
    ...
    return res.status(500).send({ data: null, message: 'Failed to get wollman' });
  }
};
