...
const getPost = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    return res.status(500).json({ data: null, message: `Failed to get post ${id}` });
  }
};
// [PUT] /api/posts/:id
const updatePost = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    posts = posts.map((post) => {
      if (post.id === Number(id)) return { ...post, title, content, updatedAt: new Date().toISOString() };
    ...
    return res.status(500).json({ data: null, message: `Post ${id} update failed` });
  }
};
// [DELETE] /api/posts/:id
const deletePost = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    return res.status(500).json({ data: null, message: `Post ${id} deletion failed` });
  }
};
