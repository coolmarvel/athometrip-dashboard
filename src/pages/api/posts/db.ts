...
export const readPostsWithUser = async (sort?: RequiredKeysOf<Post> & 'user_name', order?: Order, search?: string) => {
  try {
    const db = await readDB();
    ...
    if (search && search.length > 0) {
      posts = posts.filter((post) => {
        return post.title.toLowerCase().includes(search.toLowerCase()) || post.user?.name.toLowerCase().includes(search.toLowerCase());
      });
    }
    if (sort && order) {
