...
const getPosts = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    const posts = await readPostsWithUser(sort as RequiredKeysOf<Post> & 'user_name', order as Order);

    return res.status(200).json({ data: posts, message: 'Successfully retrieved posts' });
  } catch {
...
const getPostsByPage = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    const posts = await readPostsWithUser(sort as RequiredKeysOf<Post> & 'user_name', order as Order, search as string);
    const slicedPosts = posts.slice(Number(offset), Number(offset) + Number(limit));
...
const createPost = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    if (posts.length > 999) {
      return res.status(400).json({ data: null, message: 'Post creation limit exceeded' });
    }
    ...
    return res.status(200).json({ data: newPost.id, message: `Post ${newPost.id} created` });
  } catch {
    return res.status(500).json({ data: null, message: `Post ${title} creation failed` });
  }
};
