...
const InfiniteList = <T extends Scheme>({ infiniteQuery, renderItem, usesObserver, gap }: InfiniteListProps<T>) => {
  const { layout } = useLayout();
  const { data, isFetchingNextPage, hasNextPage, fetchNextPage } = infiniteQuery;
  ...
  const List = useMemo(() => (layout === 'horizontal' ? VirtualList : WindowVirtualList), [layout]);

  return (
    <Flex ref={containerRef} direction={'column'} overflowY={'auto'}>
      <List container={container} items={items} renderItem={renderItem} onLastItemVisible={handleLastItemVisible} gap={gap} />
      {!isFetchingNextPage ? (
        <Center>
