...
import { useUpdateOneWorld } from '@/apis';
import { OneWorldDrawer } from '@/containers';
...
const OneWorldTable = ({ oneWorld, isLoading }: OneWorldTableProps) => {
  ...
  const { mutate: updateOneWorld } = useUpdateOneWorld(queryKeyParams);

  const { openModal, openConfirm } = useModalStore(['openModal', 'openConfirm']);

  const handleDrawer = useCallback<(oneWorld: any) => void>(
    (oneWorld) => {
      if (!oneWorld) return;
      openModal(OneWorldDrawer, { oneWorld, setMutate: updateOneWorld });
    },
    [openModal, updateOneWorld]
  );
  ...
  const handleDoubleCheck = useCallback<(id: string, after: string, before: string) => void>(
    (id, after, before) => {
      openConfirm({
        ...
        onConfirm: () => updateOneWorld({ id, double_check: true, after, before }),
      });
    },
    [updateOneWorld, openConfirm, t]
  );

  const orderItemDataMap = useMemo(() => {
    return oneWorld.reduce((acc, order) => {
      const targetItem = order.line_items.find((item: any) => item.order_item_name.includes('원 월드'));
      if (targetItem) {
        acc[order.id] = {
