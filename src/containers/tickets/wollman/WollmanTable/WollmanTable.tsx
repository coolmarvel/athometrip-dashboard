...
import { useUpdateWollman } from '@/apis';
import { WollmanDrawer } from '@/containers';
...
const WollmanTable = ({ wollman, isLoading }: WollmanTableProps) => {
  ...
  const { mutate: updateWollman } = useUpdateWollman(queryKeyParams);

  const { openModal, openConfirm } = useModalStore(['openModal', 'openConfirm']);

  const handleDrawer = useCallback<(wollman: any) => void>(
    (wollman) => {
      if (!wollman) return;
      openModal(WollmanDrawer, { wollman, setMutate: updateWollman });
    },
    [openModal, updateWollman],
  );
  ...
  const handleDoubleCheck = useCallback<(id: string, after: string, before: string) => void>(
    (id, after, before) => {
      openConfirm({
        ...
        onConfirm: () => updateWollman({ id, double_check: true, after, before }),
      });
    },
    [updateWollman, openConfirm, t],
  );

  const orderItemDataMap = useMemo(() => {
    return wollman.reduce((acc, order) => {
      const targetItem = order.line_items.find((item: any) => item.order_item_name.includes('울먼'));
      if (targetItem) {
        acc[order.id] = {
