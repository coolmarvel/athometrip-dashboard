...
import { useUpdateMusicals } from '@/apis';
import { MusicalsDrawer } from '@/containers';
...
const MusicalsTable = ({ musicals, isLoading }: MusicalsTableProps) => {
  ...
  const { mutate: updateMusicals } = useUpdateMusicals(queryKeyParams);

  const { openModal, openConfirm } = useModalStore(['openModal', 'openConfirm']);

  const handleDrawer = useCallback<(musicals: OrderType) => void>(
    (musicals) => {
      if (!musicals) return;
      openModal(MusicalsDrawer, { musicals, setMutate: updateMusicals });
    },
    [openModal, updateMusicals],
  );
  ...
  const handleDoubleCheck = useCallback<(id: string, after: string, before: string) => void>(
    (id, after, before) => {
      openConfirm({
        ...
        onConfirm: () => updateMusicals({ id, double_check: true, after, before }),
      });
    },
    [updateMusicals, openConfirm, t],
  );
