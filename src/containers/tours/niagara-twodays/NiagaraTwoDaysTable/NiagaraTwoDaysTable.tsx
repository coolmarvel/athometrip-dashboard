...
import { useUpdateNiagaraTwoDays } from '@/apis';
import { NiagaraTwoDaysDrawer } from '@/containers';
...
const NiagaraTwoDaysTable = ({ niagaraTwoDays, isLoading }: NiagaraTwoDaysTableProps) => {
  ...
  const { mutate: updateNiagaraTwoDays } = useUpdateNiagaraTwoDays(queryKeyParams);

  const { openModal, openConfirm } = useModalStore(['openModal', 'openConfirm']);

  const handleDrawer = useCallback<(niagaraTwoDays: OrderType) => void>(
    (niagaraTwoDays) => {
      if (!niagaraTwoDays) return;
      openModal(NiagaraTwoDaysDrawer, { niagaraTwoDays, setMutate: updateNiagaraTwoDays });
    },
    [openModal, updateNiagaraTwoDays],
  );
  ...
  const handleDoubleCheck = useCallback<(id: string, after: string, before: string) => void>(
    (id, after, before) => {
      openConfirm({
        ...
        onConfirm: () => updateNiagaraTwoDays({ id, double_check: true, after, before }),
      });
    },
    [updateNiagaraTwoDays, openConfirm, t],
  );
