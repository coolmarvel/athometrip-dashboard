...
interface WashingtonDCModalProps {
  washingtonDC: any;
  onClose: () => void;
}

const WashingtonDCModal = ({ washingtonDC, onClose }: WashingtonDCModalProps) => {
      ...
      >
        <ModalHeader>
          <Flex justifyContent="space-between" alignItems="center">
            Order #{washingtonDC?.order.id ?? t('Order ID')}
            <Badge colorScheme={statusColor[washingtonDC?.order.status] || 'gray'} fontSize={'x-large'}>
              {washingtonDC?.order.status ? t(washingtonDC.order.status) : t('Status')}
            </Badge>
          </Flex>
        </ModalHeader>
...
export default WashingtonDCModal;
