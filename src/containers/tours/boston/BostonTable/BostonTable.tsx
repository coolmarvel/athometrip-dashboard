...
import { useUpdateBoston } from '@/apis';
import { BostonDrawer } from '@/containers';
...
const BostonTable = ({ boston, isLoading }: BostonTableProps) => {
  ...
  const { mutate: updateBoston } = useUpdateBoston(queryKeyParams);

  const { openModal, openConfirm } = useModalStore(['openModal', 'openConfirm']);

  const handleDrawer = useCallback<(boston: OrderType) => void>(
    (boston) => {
      if (!boston) return;
      openModal(BostonDrawer, { boston, setMutate: updateBoston });
    },
    [openModal, updateBoston],
  );
  ...
  const handleDoubleCheck = useCallback<(id: string, after: string, before: string) => void>(
    (id, after, before) => {
      openConfirm({
        ...
        onConfirm: () => updateBoston({ id, double_check: true, after, before }),
      });
    },
    [updateBoston, openConfirm, t],
  );
