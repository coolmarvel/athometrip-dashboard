...
import { useUpdateNiagaraOneDayAthometrip } from '@/apis';
import { NiagaraOneDayAthometripDrawer } from '@/containers';
...
const NiagaraOneDayAthometripTable = ({ niagaraOneDayAthometrip, isLoading }: NiagaraOneDayAthometripTableProps) => {
  ...
  const { mutate: updateNiagaraOneDayAthometrip } = useUpdateNiagaraOneDayAthometrip(queryKeyParams);

  const { openModal, openConfirm } = useModalStore(['openModal', 'openConfirm']);

  const handleDrawer = useCallback<(niagaraOneDayAthometrip: OrderType) => void>(
    (niagaraOneDayAthometrip) => {
      if (!niagaraOneDayAthometrip) return;
      openModal(NiagaraOneDayAthometripDrawer, { niagaraOneDayAthometrip, setMutate: updateNiagaraOneDayAthometrip });
    },
    [openModal, updateNiagaraOneDayAthometrip],
  );
  ...
  const handleDoubleCheck = useCallback<(id: string, after: string, before: string) => void>(
    (id, after, before) => {
      openConfirm({
        ...
        onConfirm: () => updateNiagaraOneDayAthometrip({ id, double_check: true, after, before }),
      });
    },
    [updateNiagaraOneDayAthometrip, openConfirm, t],
  );
