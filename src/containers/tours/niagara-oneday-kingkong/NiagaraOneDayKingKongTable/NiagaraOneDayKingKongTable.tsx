...
import { useUpdateNiagaraOneDayKingKong } from '@/apis';
import { NiagaraOneDayKingKongDrawer } from '@/containers';
...
const NiagaraOneDayKingKongTable = ({ niagaraOneDayKingKong, isLoading }: NiagaraOneDayKingKongTableProps) => {
  ...
  const { mutate: updateNiagaraOneDayKingKong } = useUpdateNiagaraOneDayKingKong(queryKeyParams);

  const { openModal, openConfirm } = useModalStore(['openModal', 'openConfirm']);

  const handleDrawer = useCallback<(niagaraOneDayKingKong: OrderType) => void>(
    (niagaraOneDayKingKong) => {
      if (!niagaraOneDayKingKong) return;
      openModal(NiagaraOneDayKingKongDrawer, { niagaraOneDayKingKong, setMutate: updateNiagaraOneDayKingKong });
    },
    [openModal, updateNiagaraOneDayKingKong],
  );
  ...
  const handleDoubleCheck = useCallback<(id: string, after: string, before: string) => void>(
    (id, after, before) => {
      openConfirm({
        ...
        onConfirm: () => updateNiagaraOneDayKingKong({ id, double_check: true, after, before }),
      });
    },
    [updateNiagaraOneDayKingKong, openConfirm, t],
  );
