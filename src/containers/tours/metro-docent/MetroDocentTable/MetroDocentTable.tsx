...
import { useUpdateMetroDocent } from '@/apis';
import { MetroDocentDrawer } from '@/containers';
...
const MetroDocentTable = ({ metroDocent, isLoading }: MetroDocentTableProps) => {
  ...
  const { mutate: updateMetroDocent } = useUpdateMetroDocent(queryKeyParams);

  const { openModal, openConfirm } = useModalStore(['openModal', 'openConfirm']);

  const handleDrawer = useCallback<(metroDocent: OrderType) => void>(
    (metroDocent) => {
      if (!metroDocent) return;
      openModal(MetroDocentDrawer, { metroDocent, setMutate: updateMetroDocent });
    },
    [openModal, updateMetroDocent],
  );
  ...
  const handleDoubleCheck = useCallback<(id: string, after: string, before: string) => void>(
    (id, after, before) => {
      openConfirm({
        ...
        onConfirm: () => updateMetroDocent({ id, double_check: true, after, before }),
      });
    },
    [updateMetroDocent, openConfirm, t],
  );
