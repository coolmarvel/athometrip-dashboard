...
import { useUpdateMetroDocentPainting } from '@/apis';
import { MetroDocentPaintingDrawer } from '@/containers';
...
const MetroDocentPaintingTable = ({ metroDocentPainting, isLoading }: MetroDocentPaintingTableProps) => {
  ...
  const { mutate: updateMetroDocentPainting } = useUpdateMetroDocentPainting(queryKeyParams);

  const { openModal, openConfirm } = useModalStore(['openModal', 'openConfirm']);

  const handleDrawer = useCallback<(metroDocentPainting: OrderType) => void>(
    (metroDocentPainting) => {
      if (!metroDocentPainting) return;
      openModal(MetroDocentPaintingDrawer, { metroDocentPainting, setMutate: updateMetroDocentPainting });
    },
    [openModal, updateMetroDocentPainting],
  );
  ...
  const handleDoubleCheck = useCallback<(id: string, after: string, before: string) => void>(
    (id, after, before) => {
      openConfirm({
        ...
        onConfirm: () => updateMetroDocentPainting({ id, double_check: true, after, before }),
      });
    },
    [updateMetroDocentPainting, openConfirm, t],
  );
