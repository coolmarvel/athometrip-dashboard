...
import { useUpdateWoodbury } from '@/apis';
import { WoodburyDrawer } from '@/containers';
...
const WoodburyTable = ({ woodbury, isLoading }: WoodburyTableProps) => {
  ...
  const { mutate: updateWoodbury } = useUpdateWoodbury(queryKeyParams);

  const { openModal, openConfirm } = useModalStore(['openModal', 'openConfirm']);

  const handleDrawer = useCallback<(woodbury: OrderType) => void>(
    (woodbury) => {
      if (!woodbury) return;
      openModal(WoodburyDrawer, { woodbury, setMutate: updateWoodbury });
    },
    [openModal, updateWoodbury],
  );
  ...
  const handleDoubleCheck = useCallback<(id: string, after: string, before: string) => void>(
    (id, after, before) => {
      openConfirm({
        ...
        onConfirm: () => updateWoodbury({ id, double_check: true, after, before }),
      });
    },
    [updateWoodbury, openConfirm, t],
  );
