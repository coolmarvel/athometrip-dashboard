...
import { useUpdateSingleDocents } from '@/apis';
import { SingleDocentsDrawer } from '@/containers';
...
const SingleDocentsTable = ({ singleDocents, isLoading }: SingleDocentsTableProps) => {
  ...
  const { mutate: updateSingleDocents } = useUpdateSingleDocents(queryKeyParams);

  const { openModal, openConfirm } = useModalStore(['openModal', 'openConfirm']);

  const handleDrawer = useCallback<(singleDocents: OrderType) => void>(
    (singleDocents) => {
      if (!singleDocents) return;
      openModal(SingleDocentsDrawer, { singleDocents, setMutate: updateSingleDocents });
    },
    [openModal, updateSingleDocents],
  );
  ...
  const handleDoubleCheck = useCallback<(id: string, after: string, before: string) => void>(
    (id, after, before) => {
      openConfirm({
        ...
        onConfirm: () => updateSingleDocents({ id, double_check: true, after, before }),
      });
    },
    [updateSingleDocents, openConfirm, t],
  );
  ...
  const table = useReactTable({ data: singleDocents, columns, getCoreRowModel: getCoreRowModel() });
