...
const ToJFkTable = ({ toJFK, isLoading }: ToJFkTableProps) => {
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor((row) => convertDate(row.jfk_shuttle_rt?.pickup_date_30 ?? row.jfk_oneway.pickup_date_10).split(' ')[0] ?? '', { header: t('pickup date') }),
      columnHelper.accessor((row) => row.jfk_shuttle_rt?.drop_add_2 ?? row.jfk_oneway?.drop_add ?? '', { header: t('boarding area') }),
