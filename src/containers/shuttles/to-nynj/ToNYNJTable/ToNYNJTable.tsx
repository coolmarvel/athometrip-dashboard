...
import { useUpdateToNYNJ } from '@/apis';
import { ToNYNJDrawer } from '@/containers';
...
const ToNYNJTable = ({ toNYNJ, isLoading }: ToNYNJTableProps) => {
  ...
  const { mutate: updateToNYNJ } = useUpdateToNYNJ(queryKeyParams);

  const { openModal, openConfirm } = useModalStore(['openModal', 'openConfirm']);

  const handleDrawer = useCallback<(toNYNJ: OrderType) => void>(
    (toNYNJ) => {
      if (!toNYNJ) return;
