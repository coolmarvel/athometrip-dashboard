...
const ToNYNightDrawer = ({ toNYNight, setMutate, onClose }: ToNYNightDrawerProps) => {
  ...
  const attributes = useMemo(
    () => [
      ...
      {
        label: t('Pickup Date'),
        value: (() => convertDate(toNYNight.order.meta_data.ewr_arrive_date ?? '').split(' ')[0])(),
      },
      { label: t('Boarding Area'), value: toNYNight.order.meta_data.ewr_pickup ?? '' },
      { label: t('Flight Number'), value: toNYNight.order.meta_data.ewr_flght_num2 ?? '' },
      {
        label: t('Memo'),
        isMemo: true,
        isEdit: isEdit,
