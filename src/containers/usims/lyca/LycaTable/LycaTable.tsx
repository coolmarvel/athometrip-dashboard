...
import { useUpdateLyca } from '@/apis';
import { LycaDrawer } from '@/containers';
...
const LycaTable = ({ lyca, isLoading }: LycaTableProps) => {
  ...
  const { mutate: updateLyca } = useUpdateLyca(queryKeyParams);

  const { openModal, openConfirm } = useModalStore(['openModal', 'openConfirm']);

  const handleDrawer = useCallback<(lyca: any) => void>(
    (lyca) => {
      if (!lyca) return;
      openModal(LycaDrawer, { lyca, setMutate: updateLyca });
    },
    [openModal, updateLyca]
  );
  ...
  const handleDoubleCheck = useCallback<(id: string, after: string, before: string) => void>(
    (id, after, before) => {
      openConfirm({
        ...
        onConfirm: () => updateLyca({ id, double_check: true, after, before }),
      });
    },
    [updateLyca, openConfirm, t]
  );

  const orderItemDataMap = useMemo(() => {
    return lyca.reduce((acc, order) => {
      const targetItem = order.line_items.find((item: any) => item.order_item_name.includes('lyca') || item.order_item_name.includes('라이카'));
      if (targetItem) {
        acc[order.id] = {
